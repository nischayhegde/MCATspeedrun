...
Rationale: students are poor judges of their own learning (Hendrick), so we never rely on self-assessment to MEASURE fluency or open gates (flashcard self-grading feeds FSRS scheduling only); the system measures fluency objectively (latency + auto-graded performance). Two scores drive everything, both computed as a LAYER ON TOP of FSRS. FSRS's per-card memory model (Stability = days for recall probability to fall to ~90%, Difficulty ~1-10, and derived Retrievability) decides WHEN a rote card returns; our layer decides WHICH mode a student is eligible for (rote/content vs application).

FLUENCY (per subtopic) = automaticity + durability of rote knowledge. It is NOT the same as FSRS stability alone. It has two components:

1. Durability: FSRS retrievability/stability for the subtopic's rote cards (is the knowledge retained over time).
2. Automaticity: response latency on CORRECT rote recalls. Fast, consistent correct recalls indicate retrieval straight from long-term memory (low working-memory load); slow-but-correct means the student is reconstructing the answer with working memory and is NOT yet fluent (Article 4). (Flashcards are self-graded, but Anki still records answer latency on every review, so automaticity is measured from time, not from the self-grade the student picked.)
   A subtopic is "fluent" only when, across >= [N] SPACED recalls, the student is both correct AND fast (below the per-type latency threshold) with FSRS retrievability >= [R_target], and there is enough evidence to be confident (see evidence rules). Recalls that happen too soon after the concept was shown (massed / high-retrievability) do NOT count toward fluency: a correct answer right after instruction is "unproductive success" and tells us nothing about retention (Article 7).
...
- Incorrect = strong, immediate evidence of non-fluency. One wrong answer is enough to (re)open rote work for that subtopic.
- A single fast-correct = WEAK evidence. On 4-option MCQs a fast-correct can be a lucky guess (~25%) or unproductive success. We therefore require CONSISTENCY across multiple spaced reviews (a confidence threshold) before declaring fluency, never a single data point.
...
- Bidirectional inference: demonstrated application IMPLIES fluency, so a student who solves application problems (in the diagnostic or later) is credited as fluent and SKIPS rote drilling for that subtopic. We never force a strong student through flashcards they don't need.
...
SCORE DYNAMICS: fluency durability decays naturally via FSRS retrievability. The automaticity and application components are computed over a recency-weighted rolling window of attempts (exponential decay, tau = 21 days), and confidence grows with the number of spaced attempts.

SPACING WEIGHTS (continuous, not a massed/spaced cliff — 2026-07-01 spec): every attempt's evidence weight scales with the gap since the item (rote) or subtopic (application) was last practiced. Rote: weight = clamp01(elapsed_days / 3), first-ever exposure = 1. Application: weight = max(0.3, clamp01(leaf_gap_days / 1)) — same-day fresh problems are distinct items, so they keep a floor (this also lets the diagnostic seed estimates). Per-leaf evidence depth ("attempts") is the SUM of these weights, so ten crammed repeats never read as ten spaced data points. The fluency gate needs effective spaced-correct >= 2.0 (not a raw count).

EXPECTED TIME PER ITEM (tediousness-adjusted latency): a slow answer only signals non-fluency if the item shouldn't take that long. The parser scores each question's reasoning_complexity (rc) and calculation_tedium (ct) 1-5; the importer tags them (mcat::rc::N / mcat::ct::N) and the per-kind latency thresholds are scaled by 1 + 0.15*(rc-3) + 0.15*(ct-3), clamped to [0.5, 2.0]. Flashcards are exempt — fast facts must be fast. This feeds both auto-grading (Hard/Good/Easy) and the automaticity/application speed signals.
...
Parameters to calibrate: [N] spaced recalls for fluency, [R_target], per-type latency thresholds, confidence threshold for declaring fluency, diagnostic size (fixed at 120 total; items-per-subtopic follows from taxonomy size), and the split of the finite ~2500-item bank between the diagnostic and application pools (plus behavior when a subtopic's application pool is exhausted).

GRADING & RATING--------------------------------------------------------------------------
Two grading paths, by item type:

FLASHCARDS (rote) - SELF-GRADED (standard Anki): after revealing the back, the student presses one of the four buttons (Again / Hard / Good / Easy). This is the native Anki flow, and FSRS consumes these ratings directly for scheduling. We accept self-grading here because term->description recall has no machine-checkable answer and typing every answer would add too much friction. Caveat: self-assessment is unreliable (Hendrick), so a flashcard "Easy" alone NEVER opens the fluency gate; the gate needs objective corroboration (below).
...
OBJECTIVE FLUENCY SIGNAL: Anki records answer latency (taken_millis) on EVERY review, including self-graded flashcards. So the automaticity component of fluency uses measured latency, not the button the student pressed. The fluency gate opens only on objective corroboration: fast, consistent, spaced correct recalls (latency-based) plus performance on auto-graded discrete items - never on a student clicking "Easy."

Caveats (see Scoring Model): (1) a fast-correct on an auto-graded item is only PROVISIONAL evidence; corroborate across spaced reviews (a 4-option MCQ can be guessed). (2) A wrong answer on FIRST exposure (diagnostic or first attempt) is PRODUCTIVE FAILURE routed to instruction, not an FSRS lapse; only post-learning failures are true lapses. Application problems are graded for correctness/time but never re-scheduled as the same item (fresh problems only).
...
- Rote flashcard (latency only, for the automaticity signal - NOT used to set the button, which the user picks): fast < [Xs], slow > [Ys]
...
6. Limits/order: sort by the `due` priority, then apply caps - PER_LEAF_MAX per subtopic, StudyConfig.new_per_day for new cards, StudyConfig.session_size total.
   DEMOTION (lapse -> rote): if a fluent subtopic lapses (a genuine post-learning miss - detected in aggregate.rs), its fluency gate CLOSES; the student resumes new+due rote for it until re-fluent, and application practice pauses for that subtopic.
...
- SHRINKAGE: the raw estimate can't tell a lucky fast guess from real skill, so it is pulled toward a low neutral prior in proportion to how little evidence backs it: n_i = attempts_i * freshness_i (effective spaced evidence, stale-discounted); w = n/(n+5); m_adj = w*m_i + (1-w)*0.1. Cold collection reads ~478, not 472; thin evidence never reads as mastered; with deep evidence m_adj converges to the raw score.
...
4. Study (ts/routes/mcat/study, StudyPage.svelte): ONE unified, interleaved queue (no separate flashcard/application tabs). Each item renders in its own mode - FLASHCARD: front -> reveal -> SELF-GRADE Again/Hard/Good/Easy (keys 1-4); MCQ: parsed question image + letter-only A/B/C/D (keys A-D), then inline feedback revealing correct letter + explanation and a Continue. Progress bar; latency captured for both; on completion routes back to the Dashboard. NO session-summary screen.
...
6. Settings: boxer animation on/off, daily new/review limits, advanced latency-threshold overrides (hidden by default). [planned]
   [RESOLVED: rote flashcards use standard SELF-GRADING (Again/Hard/Good/Easy); only application/MCQ items are auto-graded (choice + latency). Both modes live in the single StudyPage; flashcard latency is still recorded for the automaticity signal. See GRADING & RATING.]
...
- `mcat::diff::<N>` carries overall difficulty 1-5 (drives the diagnostic difficulty spread).
  ...
  PER-CARD STATE: FSRS memory state (existing revlog + card memory) is the source of truth; per-attempt signals (correct?, latency, productive-failure/massed) are reconstructed from the review log in adapter.rs rather than requiring a new per-attempt table.
...
- Grading: MCQs auto-graded from correctness + latency; flashcards self-graded; both flow through FSRS. Built on Anki's real FSRS scheduler (not a parallel scheduler) - FSRS owns rote scheduling; our layer adds the fluency/application/gate scoring on top.
  ...
  NOT YET BUILT: 8-bit boxer animation, onboarding screen, standalone tag-browser + settings screens, LLM explanation grading (all noted inline above).
