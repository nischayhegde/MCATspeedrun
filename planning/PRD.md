...
OBJECTIVE FLUENCY SIGNAL: Anki records answer latency (taken_millis) on EVERY review. The automaticity component of fluency uses measured latency, and flashcard correctness is now the LLM verdict (objective), not a button the student pressed. The fluency gate opens only on objective corroboration: fast, consistent, spaced correct recalls (latency + verdict) plus performance on auto-graded discrete items - never on a student self-declaring mastery.
